...
/*   Updated: 2025/08/17 14:31:25 by clumertz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_child(t_process *p_1, t_process *p_2)
{
	...
	if (p_1->pid == -1)
		...
		child_one(p_1, p_2, fd);
	p_2->pid = fork();
	if (p_2->pid == -1)
		close_fd(fd);
	else if (p_2->pid == 0)
		child_two(p_2, p_1, fd);
	wait_process(p_1, p_2, fd);
}

void	child_one(t_process *p_1, t_process *p_2, int *fd)
{
	close(fd[0]);
	close(p_2->fd_file);
...
void	child_two(t_process *p_2, t_process *p_1, int *fd)
{
	close(fd[1]);
	close(p_1->fd_file);
